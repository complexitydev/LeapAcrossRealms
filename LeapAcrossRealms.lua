RealmHopAssist = LibStub("AceAddon-3.0"):NewAddon("RealmHopAssist", "AceEvent-3.0", "AceConsole-3.0", "AceTimer-3.0")
local addon = RealmHopAssist;
...
local homeRealm, zoneName, realmSep, gui, btRefresh, btSettings, btQuick, btManual, lbRealm, lbStatus, playerName, lfgui, lfgTabSelected, lfgTable, lbThrottle, curLfgGroup, sheduledScan, wasInGroup, inInstance
local settingsMenu
local hintShown
...
local SettingsDefaults={
    global={
        ...
        quickJoin={[6]=1},
        allLanguages = true,
        stoplist = {"no hop","nohop","no realm","kick","no rha","norha","no rh","norh","no realm hop","no realmhop","norealmhop"},
        priorityList = {"realm hop","realmhop","garrison","rha","rh","hop"},
...
function addon.ShowMmTooltip(tooltip)
    tooltip:AddLine("Realm Hop Assist",1,1,1)
    tooltip:Show()
end

function addon:OnInitialize()
    db = LibStub("AceDB-3.0"):New("RealmHopAssistDB", SettingsDefaults)
    addon.db = db;
    local minimapData = ldb:NewDataObject("RealmHopAssistMinimapIcon",{
        type = "data source",
        text = "Realm Hop Assist",
        icon = "Interface/Icons/INV_Misc_GroupNeedMore",
        OnClick = addon.Activate,
        OnTooltipShow = addon.ShowMmTooltip
    })
    minimapIcon:Register("RealmHopAssistMinimapIcon", minimapData, db.global.minimap)
end
...
function addon:OnEnable()
    ...
    -- Old commands --
    addon:RegisterChatCommand("cra", "OldCommandReminder")
    addon:RegisterChatCommand("crossrealmassist", "OldCommandReminder")
    -- Old commands --
    addon:RegisterChatCommand("rha", "Activate")
    addon:RegisterChatCommand("rhahop", "Activate")
    addon:RegisterChatCommand("realmhopassit", "Activate")
    if db.global.autoCreateHopGroup then
        addon:RegisterEvent("PLAYER_FLAGS_CHANGED", "PlayerFlagsUpdate")
    end
end
-- Old Commands Reminder

function addon:OldCommandReminder()
    DEFAULT_CHAT_FRAME:AddMessage("|cffff0000Type |cffffffff/rha |cffff0000in chat to open Realm Hop Assist without minimap button");
end
...
function addon:CreateUI()
    ...
    gui = setupWidget(CreateFrame("Frame","RealmHopAssistMainUI",nil,"InsetFrameTemplate3"), {SetFrameStrata="LOW",SetWidth=208,SetHeight=60,EnableMouse=true,SetMovable=true,SetClampedToScreen=true,SetScale=scale})
...
function addon:CreateLFGUI()
    ...
    lfgui = setupWidget(CreateFrame("Frame","RealmHopAssistJoinUI",nil,"UIPanelDialogTemplate"), {SetFrameStrata="HIGH",SetWidth=tableWidth+45,SetHeight=shift+95,EnableMouse=true,SetMovable=true,SetScale=scale})
    ...
    btFilterCount = CreateFrame("FRAME", "RealmHopAssistCountFilter", lfgui, "UIDropDownMenuTemplate")
...
local function CreateHopGroup()
    if canJoinGroup() then
        --C_LFGList.CreateListing(16,"Realm hopping - "..homeRealm,0,0,"","Realm Hop Assist - Group (Questing/Farming) - "..homeRealm.." - "..zoneName,true,false)
    end
end

local function CreateHopRaid()
    if canJoinGroup() then
        --C_LFGList.CreateListing(lfgID, itemLevel, honorLevel, autoAccept, privateGroup[, questID])
        --C_LFGList.CreateListing(16,"Realm hopping-"..homeRealm,0,0,"","Realm Hop Assist - Raid (Farming) - "..homeRealm.." - "..zoneName,true,false)
...
function addon.toggleMinimapIcon(hidden)
    if hidden then
        DEFAULT_CHAT_FRAME:AddMessage("|cffff0000Type |cffffffff/rha |cffff0000in chat to open Realm Hop Assist without minimap button");
        minimapIcon:Hide("RealmHopAssistMinimapIcon")
    else
        minimapIcon:Show("RealmHopAssistMinimapIcon")
    end
end
...
local function initMenu(self, level)
    ...
    if level == 1 then
        ...
        UIDropDownMenu_AddButton({text="Realm Hop Assist Toolbox",notCheckable=1,func=CreateHopGroup,isTitle=1}, level)
		...
		if canJoinGroup() then
            UIDropDownMenu_AddButton({text="Create a group for realm hoppers",disabled=1,notCheckable=1,func=CreateHopGroup}, level)
            UIDropDownMenu_AddButton({text="Create a raid for realm hoppers",disabled=1,notCheckable=1,func=CreateHopRaid}, level)
...
function addon.ShowSettings(frame)
    if not settingsMenu then
        settingsMenu = CreateFrame("Frame", "RealmHopAssistMenu")
